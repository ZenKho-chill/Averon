...
- `modules/webui` (1.0.0 → 1.1.0): **nâng cấp dashboard admin + người dùng**. (VI)
  ...
  - Kèm test mới: `logs.test.ts` (8 test LogTailer), mở rộng `api.test.ts`/`server.test.ts` (usage/guilds).
  - EN: **Enhanced admin + user dashboards.** Realtime log streaming over WebSocket via a new `LogTailer` (tails the newest `logs/` file; handles rotation/truncate/incomplete lines) pushed to all admin sockets with the status+modules snapshot every 3s. New **Usage** tab: aggregates core command-usage log lines → total + by module/command/guild (`GET /api/admin/usage`). Richer **user dashboard**: per shared guild show icon, member count, "you manage this guild" badge (`ManageGuild` permission) and a guild-prefilled "Invite bot" link. WS now uses a single shared broadcast ticker. New tests: `logs.test.ts`, extended `api.test.ts`/`server.test.ts`.

### Removed
- `modules/webui`: gỡ khỏi dashboard admin **tab Config** (sửa core/module config qua web — tính năng 3.5.0), **tab Backups** (liệt kê + khôi phục backup config) và **tab Crash** (xem crash reports) — giao diện gọn hơn, admin chỉnh config qua file. Bỏ luôn các endpoint đi kèm (`/api/admin/config`, `/api/admin/backups`, `/api/admin/backups/restore`, `/api/admin/crash-reports(/:file)`) + handler + test tương ứng. (VI)
  EN: Removed the **Config** tab (web-based core/module config editing — a 3.5.0 feature), the **Backups** tab (list/restore config backups) and the **Crash** tab (view crash reports) from the admin dashboard for a leaner UI — config is edited via files. Also removed the related endpoints, handlers and tests.

## [3.5.0] — 2026-08-15
**Loại / Type:** MINOR — thêm tính năng mới / new feature

### Added
- `modules/webui`: **module mới — Web UI (trang chủ + dashboard admin & người dùng)**. HTTP server thuần (`in-process`): trang chủ public (status + modules realtime qua WebSocket `ws`), dashboard system admin (xem status/modules/config/logs, load/unload/reload module, sửa core/module config — hiển thị được mask secret, lưu lại được validate lại schema), dashboard user (Discord OAuth2 → danh sách guild dùng chung bot ↔ user). **Auth 100% qua Discord OAuth2** (không có API token): admin = user trong `admin_user_ids`; trang chủ có nút "Admin dashboard" → `/oauth2/login`. **Secret** (`webui.oauth2.client_secret`) đặt trong `config/config.yml` (gitignored, repo PUBLIC); non-secret trong `config/defaults.yml` (tracked). Bảo mật: không trả secret ra web (mask theo `token|secret|password`), anti-CSRF state, chống path traversal, host non-localhost thiếu OAuth2 → fail-fast. Kèm 52 test (config/auth/api/server/ws). README module + `config/config.example.yml` mục `webui`. (VI)
  EN: New `modules/webui` — **Web UI (homepage + admin & user dashboard)**. A plain HTTP server (`in-process`): public homepage (status + modules in realtime via `ws` WebSocket), system-admin dashboard (status/modules/config/logs, load/unload/reload modules, edit core/module config — masked secrets, re-validated against schema before saving), user dashboard (Discord OAuth2 → guilds shared between bot and user). **Auth is 100% Discord OAuth2** (no API token): admins are users in `admin_user_ids`; homepage "Admin dashboard" button → `/oauth2/login`. **Secrets** (`webui.oauth2.client_secret`) live in `config/config.yml` (gitignored, public repo); non-secrets in `config/defaults.yml` (tracked). Security: secrets never sent to the web (masked by `token|secret|password`), CSRF state, path-traversal guard, non-localhost host without OAuth2 → fail-fast. 52 tests (config/auth/api/server/ws). Module README + `webui` section in `config/config.example.yml`.
- `core/registry` + `core/lifecycle` + `core/bootstrap`: **mở rộng service API cho module** — `CoreServices` thêm `manager`, `discord`, `usage`, `registry`, `root` (ngoài `logger`, `config`); `RegistryLike` thêm `getService<K>(key)` (type-safe, §5.3) để module lấy service core mà **không import core internal**. Bootstrap register các service **trước** `manager.loadAll()` (module load có thể dùng ngay). Hook `onLoad` mở rộng: `onLoad(registry?: RegistryLike)` — module cũ `onLoad()` vẫn hoạt động (backward-compatible). Cập nhật `docs/api/README.md` (§1a). Kèm test registry/bootstrap (VI)
  EN: `core/registry` + `core/lifecycle` + `core/bootstrap`: **extended the module service API** — `CoreServices` now exposes `manager`, `discord`, `usage`, `registry`, `root` (in addition to `logger`, `config`); `RegistryLike` gains `getService<K>(key)` (type-safe, §5.3) so modules can fetch core services **without importing core internals**. Bootstrap registers the services **before** `manager.loadAll()` (ready for module load). The `onLoad` hook is extended: `onLoad(registry?: RegistryLike)` — legacy `onLoad()` still works (backward-compatible). Updated `docs/api/README.md` (§1a). Registry/bootstrap tests added.
