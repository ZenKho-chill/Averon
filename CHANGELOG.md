...
### Added
- `modules/webui`: **module mới — Web UI (trang chủ + dashboard admin & người dùng)**. HTTP server thuần (`in-process`): trang chủ public (status + modules realtime qua WebSocket `ws`), dashboard system admin (xem status/modules/config/logs, load/unload/reload module, sửa core/module config — hiển thị được mask secret, lưu lại được validate lại schema), dashboard user (Discord OAuth2 → danh sách guild dùng chung bot ↔ user). Admin xác thực bằng API token (timing-safe) hoặc Discord OAuth2 (`admin_user_ids`). **Secret** (`webui.api_token`, `webui.oauth2.client_secret`) đặt trong `config/config.yml` (gitignored, repo PUBLIC); non-secret trong `config/defaults.yml` (tracked). Bảo mật: không trả secret ra web (mask theo `token|secret|password`), anti-CSRF state, cookie `HttpOnly`, chống path traversal, host non-localhost thiếu auth → fail-fast. Kèm 46 test (config/auth/api/server/ws). README module + `config/config.example.yml` mục `webui`. (VI)
  EN: New `modules/webui` — **Web UI (homepage + admin & user dashboard)**. A plain HTTP server (`in-process`): public homepage (status + modules in realtime via `ws` WebSocket), system-admin dashboard (status/modules/config/logs, load/unload/reload modules, edit core/module config — masked secrets, re-validated against schema before saving), user dashboard (Discord OAuth2 → guilds shared between bot and user). Admin auth via API token (timing-safe) or Discord OAuth2 (`admin_user_ids`). **Secrets** (`webui.api_token`, `webui.oauth2.client_secret`) live in `config/config.yml` (gitignored, public repo); non-secrets in `config/defaults.yml` (tracked). Security: secrets never sent to the web (masked by `token|secret|password`), CSRF state, `HttpOnly` cookie, path-traversal guard, non-localhost host without auth → fail-fast. 46 tests (config/auth/api/server/ws). Module README + `webui` section in `config/config.example.yml`.
